...
interface MoreFeatureDao {
    ...
    suspend fun insertBlade(blade: MoreFeature)

    @Delete
    suspend fun deleteBlade(blade: MoreFeature)
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertMotor(motor: MoreFeature)

    @Delete
    suspend fun deleteMotor(motor: MoreFeature)
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertShaft(shaft: MoreFeature)

    @Delete
    suspend fun deleteShaft(shaft: MoreFeature)
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertBox(box: MoreFeature)

    @Delete
    suspend fun deleteBox(box: MoreFeature)

    // امکانات بیشتر
    @Query(
        "SELECT * FROM more_features " +
        "WHERE name NOT LIKE 'تیغه%' AND name NOT LIKE 'موتور%' " +
        "AND name NOT LIKE 'شفت%' AND name NOT LIKE 'قوطی%'"
    )
    fun getExtras(): Flow<List<MoreFeature>>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertExtra(feature: MoreFeature)

    @Delete
    suspend fun deleteExtra(feature: MoreFeature)
}
